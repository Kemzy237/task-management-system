...
if (isset($_POST['title']) && isset($_POST['description']) && isset($_POST['assigned_to']) && $_SESSION['role'] == 'admin' && isset($_POST['due_date'])) {
	...
	if (empty($title)) {
	    ...
	    header("Location: ../create_task-department.php?error=$em");
	    exit();
		...
		$notif_data = array("'$title' has been assigned to you. Please review and start working on it",
		 $assigned_to, 'New Task Assigned');
        insert_notification($conn, $notif_data);

        $em = "Task Created Succesfully";
	    header("Location: ../create_task-department.php?success=$em");
	    exit();
	}
}else {
